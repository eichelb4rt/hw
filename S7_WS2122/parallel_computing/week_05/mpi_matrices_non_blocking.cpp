...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        ...
        MPI_Request* requests = (MPI_Request*) malloc(3 * problem_size * sizeof(MPI_Request));
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
                int task_element = i * n + j;
                int tasked_rank = (task_element % working_processes) + 1;
                // C[i][j] is calculated with row i of matrix A and column j of matrix B (row j of B^T)
                MPI_Isend(matrix_a[i], r, MPI_DOUBLE, tasked_rank, 3 * task_element + 0, MPI_COMM_WORLD, requests + 3 * task_element + 0);
                MPI_Isend(matrix_b_T[j], r, MPI_DOUBLE, tasked_rank, 3 * task_element + 1, MPI_COMM_WORLD, requests + 3 * task_element + 1);
            }
        }
        ...
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
                ...
                int tasked_rank = (task_element % working_processes) + 1;
                // C[i][j] is calculated with row i of matrix A and column j of matrix B (row j of B^T)
                MPI_Irecv(&matrix_c[i][j], 1, MPI_DOUBLE, tasked_rank, 3 * task_element + 2, MPI_COMM_WORLD, requests + 3 * task_element + 2);
            }
        }

        // wait for all the requests
        MPI_Waitall(3 * problem_size, requests, MPI_STATUS_IGNORE);
        ...
        // calculate how many time each process has to calculate something
        int whole_waves = problem_size / working_processes;
        int rest_wave = problem_size % working_processes;
        int n_tasks = rank > rest_wave ? whole_waves : whole_waves + 1;

        MPI_Request* send_reqs = (MPI_Request*) malloc(n_tasks * sizeof(MPI_Request));
        for (int task = 0; task < n_tasks; ++task) {
            // careful with the rest wave!
            int completed_tasks = task * working_processes;
            int task_element = completed_tasks + rank - 1;
            // get row i from matrix A and column j from matrix B, calculate C[i][j], send back the result
            MPI_Recv(a_row, r, MPI_DOUBLE, 0, 3 * task_element + 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv(b_column, r, MPI_DOUBLE, 0, 3 * task_element + 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            double result = calc_block(r, a_row, b_column);
            MPI_Isend(&result, 1, MPI_DOUBLE, 0, 3 * task_element + 2, MPI_COMM_WORLD, send_reqs + task);
        }
        MPI_Waitall(n_tasks, send_reqs, MPI_STATUS_IGNORE);
    }
