...
void send_ghosts(int neighbour_rank, double* grid, int start_index, MPI_Datatype border_type, int* array_of_requests, int* current_request) {
    int TAG = *current_request;
    if (neighbour_rank == UNDEFINED_RANK) return;
    // send from non-ghost-zone-end - g = chunk_width + g - g
    MPI_Isend(&grid[start_index], 1, border_type, neighbour_rank, TAG,
        MPI_COMM_WORLD, &array_of_requests[*current_request]);
    // update current index because communication was succesful
    ++(*current_request);
}

void recv_ghosts(int neighbour_rank, double* grid, int start_index, MPI_Datatype border_type, int* array_of_requests, int* current_request) {
    if (neighbour_rank == UNDEFINED_RANK) return;
    // receive in ghost-zone
    MPI_Irecv(&grid[start_index], 1, border_type, neighbour_rank, MPI_ANY_TAG,
        MPI_COMM_WORLD, &array_of_requests[*current_request]);
    // update current index because communication was succesful
    ++(*current_request);
}